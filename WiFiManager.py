...
class WiFiManager:
    ...
    def EnsureConnection(self):
        if (self.usingSpi):
            while not self.esp.is_connected:
                try:
                    print("Ensuring WiFi Connection to " + secrets["ssid"] + ".")
                    self.esp.connect_AP(secrets["ssid"], secrets["password"])
                    print("WiFi Connected to " + secrets["ssid"] + ".")
                except RuntimeError as e:
                    print("could not connect to AP: " + secrets["ssid"] + ", retrying: ", e)
                    time.sleep(2)
        else:
    ...
    def get_data_source_url(self, api="onecall", location=None):
    ...
    def get_latlon(self):
        self.EnsureConnection()
        JSON_URL = self.get_data_source_url(api="forecast5", location=secrets["openweather_location"])
        r = None
        if (self.usingSpi):
            r = adafruit_requests.get(JSON_URL)
        else:
    ...
    def get_forecast(self, location):
        self.EnsureConnection()
        JSON_URL = self.get_data_source_url(api="onecall", location=location)
        r = None
        if (self.usingSpi):
            r = adafruit_requests.get(JSON_URL)
        else:
